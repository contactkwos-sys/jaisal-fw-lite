/**
 * Design OCR parser tests — Format A/B + Colour 1/2/3 (112-pick) sheets.
...
const COLOUR_ROW_RE = /^(?:colour|color|col\.?)\s*(\d+)\s*(?:[|:.\-]\s*|\s+)(.*)$/i
...
function extractColourTable(text) {
  ...
  for (const line of lines) {
    ...
    if (nums.length >= 2) {
      ...
      let yarnRaw = rest
      const lastTwo = new RegExp(`${pic.replace('.', '\\.')}\\s+${strings.replace('.', '\\.')}\\s*$`)
...
assert(colour.weftRows[1].pic === '37', 'Colour 2 pick')
assert(colour.weftRows[2].pic === '37', 'Colour 3 pick')
