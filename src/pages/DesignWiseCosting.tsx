...
export function DesignWiseCosting({ initialDin = '', viewOnly = false, onNavigate }: Props) {
  ...
  async function persist(asDraft: boolean, finalize = false, overrides?: PersistOverrides) {
    ...
    try {
      ...
      setMessage(
        finalize
          ? `Finalized · CEO rate ${fmtInr(n(ceoFinalSellingRate) || totals.finalCostPerMtr)}/mtr`
          : asDraft
            ? `Draft saved · ${fmtInr(totals.finalCostPerMtr)}/mtr`
            : `Costing saved · ${fmtInr(totals.finalCostPerMtr)}/mtr`,
      )
    } catch (e) {
