-- Digital Factory Notebook — additive module (notes, attachments, purchase photos)
...
-- Purchase photos kept separate from general notebook notes
...
begin
  if not exists (
    ...
    where schemaname = 'storage' and tablename = 'objects' and policyname = 'notebook_photos_read'
  ) then
    create policy notebook_photos_read on storage.objects
      for select to public
      using (bucket_id = 'notebook-photos');
  end if;
  if not exists (
    select 1 from pg_policies
    where schemaname = 'storage' and tablename = 'objects' and policyname = 'notebook_photos_write'
  ) then
    create policy notebook_photos_write on storage.objects
