-- DIN Costing (Jacquard Repair Design) — formula master, wastage, CEO selling rate, audit

-- Formula Master (singleton — CEO/Admin editable)
...
comment on column public.design_costing.design_length_mtr is
  'Entered production / base length (mtr) — yarn rows consume on this length';
comment on column public.design_costing.usable_length_mtr is
  'Usable length for costing basis (entered − wastage)';
comment on column public.design_costing.ceo_final_selling_rate is
  'CEO-finalized commercial selling rate per meter — separate from calculated cost';
comment on column public.design_costing.is_locked is
  'True after Finalize Costing — only CEO/Admin can reopen';

-- Backfill wastage / usable from design length where missing
...
-- Audit trail for costing changes
...
-- Extend status check to include locked final
