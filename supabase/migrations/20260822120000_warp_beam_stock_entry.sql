-- Warp Beam Stock Entry — item master, machine entries, gate passes (additive)
-- Extends warp_yarn_management without dropping existing data.
...
-- Backfill items from existing warp_pipes yarn_quality
...
create index if not exists warp_machine_entries_pipe_idx
  on public.warp_machine_entries (pipe_no);

-- ---------- Per-machine lines in an entry ----------
...
create index if not exists warp_machine_entry_lines_entry_idx
  on public.warp_machine_entry_lines (entry_id);
create index if not exists warp_machine_entry_lines_machine_idx
  on public.warp_machine_entry_lines (machine_no);

-- ---------- Warp gate passes (warper / job-work) ----------
...
create index if not exists warp_gate_passes_date_idx
  on public.warp_gate_passes (pass_date desc);
create index if not exists warp_gate_passes_party_idx
  on public.warp_gate_passes (party_name);
create index if not exists warp_gate_passes_pipe_idx
  on public.warp_gate_passes (pipe_no);

-- Extend warp_pipes with pipe_type for empty pipes
