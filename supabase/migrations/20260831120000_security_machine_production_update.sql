...
-- Simple Security-operator shift entry → ERP dashboard sync.
-- Does not alter existing production_entries schema beyond optional source tagging.

-- ---------- Operator master (permanent names for Security screen) ----------
...
create table if not exists public.security_shift_machines (
  ...
  stop_reason text,
  -- Electronic Fault | Mechanical Fault | Operator Problem
  operator_name text,
...
-- Optional source tag on production_entries (idempotent)
alter table public.production_entries
  add column if not exists source text;

comment on column public.production_entries.source is
  'Origin of entry: security_mobile | machine_wise | classic | null';
...
-- Also allow anon for PIN-session clients that use anon key with JWT
